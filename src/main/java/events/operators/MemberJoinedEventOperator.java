...
public class MemberJoinedEventOperator {
	...
	public void processMemberJoinedEvent(@Nonnull GuildMemberJoinEvent event) {
		...
		Channels.getPwiKingdomGeneralChannel()
				.sendMessage("Welcome to PWI Kingdom **" + username + "**! Make sure to read <#617691793315725317> to learn the rules and other useful information! <:ttcute:288020411126644736>")
				.queue();
	 ...
	 */
	private boolean verifyAndProcessUser(GuildMemberJoinEvent event, @Nullable PrivateChannel privateChannel, String username) {
	...
	private void kickAndReport(String username, Member member) {
		error.Reporter.reportToPWIKingdom(username + " was kicked due to having advertisement in the username.");
