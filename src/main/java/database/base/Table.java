...
public abstract class Table {
	...
	/**
	 * Loads the database info into the memory.
	 */
	 ...
	 * <br><br>Prints error message to console in case of failure.
	 */
	@CheckForNull
	...
	protected boolean add__Internal(Object... values) {
		try {
			...
			e.printStackTrace();
			error.Reporter.report("Failed to add row on table " + tableName);
			return false;
		}
		return true;
	}
	...
	protected Integer remove__Internal(String[] valuesNames, Object... values) {
		...
		try {
			...
			e.printStackTrace();
			error.Reporter.report("Failed to remove row on table " + tableName);
			return null;
		}
	}
	...
	private boolean setStatementValues(PreparedStatement stm, Object... values) {
		try {
			...
			e.printStackTrace();
			error.Reporter.report("Failed to set statement values on table " + tableName);
			return false;
		}
		return true;
	}
